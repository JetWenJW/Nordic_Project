...
/* Set Random Static Address to the advertising Device */
static void set_random_static_address(void)
{
  ...
  rs_addr.addr[3] = 0x66;
  rs_addr.addr[4] = 0x33;
  rs_addr.addr[5] = 0xFF;  /* MSB Must be 11 */ 

  rs_addr.addr_type = BLE_GAP_ADDR_TYPE_RANDOM_STATIC;
  err_code = sd_ble_gap_addr_set(&rs_addr);
  if(err_code != NRF_SUCCESS)
  {
    NRF_LOG_INFO("Fail to set the Random Static address");
  }
}

/* Get Random Static Address to the advertising Device */
static void get_random_static_address(void)
{
  ...
  if(err_code == NRF_SUCCESS)
  {
    ...
    NRF_LOG_INFO("Device Address is : %02x : %02x : %02x : %02x : %02x : %02x",
                  my_device_addr.addr[0], my_device_addr.addr[1], my_device_addr.addr[2],  
                  my_device_addr.addr[3], my_device_addr.addr[4], my_device_addr.addr[5]
                );
  }
}
