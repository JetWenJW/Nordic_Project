...
/* Define Check Address Timer */
#define ADDER_CHECK_TIME_INTERVAL       APP_TIMER_TICKS(9000)   /* 9sec */
...
APP_TIMER_DEF(m_check_addr_timer_id);/* Set timer to check PDU */
...
/* Create Timer Handler to check PDU */
static void check_addr_timeout_handler(void *p_context)
{
  ret_code_t err_code;
  static ble_gap_addr_t my_addr;

  err_code = sd_ble_gap_adv_addr_get(m_advertising.adv_handle, &my_addr);
  if(err_code == NRF_SUCCESS)
  {
    NRF_LOG_INFO("Address Type : %02x", my_addr.addr_type);
    NRF_LOG_INFO("Device Address is : %02x : %02x : %02x : %02x : %02x : %02x",
                  my_addr.addr[0], my_addr.addr[1], my_addr.addr[2],
...
static void timers_init(void)
{
  ...
  /* Set Timer to check PDU */
  err_code = app_timer_create(&m_check_addr_timer_id,
                              APP_TIMER_MODE_REPEATED,
                              check_addr_timeout_handler);
  APP_ERROR_CHECK(err_code);
}
...
/* Set The Non-Resolvable Private Address */
static void set_non_resolvable_private_address(void)
{
  uint32_t err_code;
  static ble_gap_privacy_params_t dev_addr = {0};

  dev_addr.privacy_mode         = BLE_GAP_PRIVACY_MODE_DEVICE_PRIVACY;
  dev_addr.private_addr_type    = BLE_GAP_ADDR_TYPE_RANDOM_PRIVATE_NON_RESOLVABLE;
  dev_addr.private_addr_cycle_s = 15;
  dev_addr.p_device_irk         = NULL;
  
  err_code = sd_ble_gap_privacy_set(&dev_addr);
  if(err_code != NRF_SUCCESS)
  {
    NRF_LOG_INFO("Fail to set Non-Resolvable Private Address...");
  }
}
...
int main(void)
{
    ...
    set_non_resolvable_private_address();

    advertising_start();

    uint32_t err_code = app_timer_start(m_check_addr_timer_id, ADDER_CHECK_TIME_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
